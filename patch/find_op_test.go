...
var _ = Describe("FindOp.Apply", func() {
	It("returns document if path is for the entire document", func() {
		res, err := FindOp{Path: MustNewPointerFromString("")}.Apply(CreateYamlNodeV2("a"))
		Expect(err).ToNot(HaveOccurred())
		Expect(res).To(Equal(CreateYamlNodeV2("a")))
	})

	Describe("array item", func() {
		It("finds array item", func() {
			res, err := FindOp{Path: MustNewPointerFromString("/0")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(1)))

			res, err = FindOp{Path: MustNewPointerFromString("/1")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(2)))

			res, err = FindOp{Path: MustNewPointerFromString("/2")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(3)))
		...
		It("finds relative array item", func() {
			res, err := FindOp{Path: MustNewPointerFromString("/3:prev")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(3)))
		})
		...
		It("finds nested array item", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(11)))
		})
		...
		It("returns an error if it's not an array when index is being accessed", func() {
			_, err := FindOp{Path: MustNewPointerFromString("/0")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/0' but found '*yamltree.ClassicMappingNode'"))

			_, err = FindOp{Path: MustNewPointerFromString("/0/1")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/0' but found '*yamltree.ClassicMappingNode'"))
		...
		It("returns an error if the index is out of bounds", func() {
			_, err := FindOp{Path: MustNewPointerFromString("/1")}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal(
				"Expected to find array index '1' but found array of length '0' for path '/1'"))

			_, err = FindOp{Path: MustNewPointerFromString("/1/1")}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal(
				"Expected to find array index '1' but found array of length '0' for path '/1'"))
		})
	...
	Describe("array with after last item", func() {
		It("returns an error as after-last-index tokens are not supported", func() {
			_, err := FindOp{Path: MustNewPointerFromString("/-")}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal(
		...
		It("returns an error if after last index token is not last", func() {
			...
			_, err := FindOp{Path: ptr}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal(
				"Expected not to find after last index token in path '/-/' (not supported in find operations)"))
		})
		...
		It("returns an error if it's not an array being accessed", func() {
			_, err := FindOp{Path: MustNewPointerFromString("/-")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(Equal(
				"Expected not to find after last index token in path '/-' (not supported in find operations)"))
	...
	Describe("array item with matching key and value", func() {
		It("finds array item if found", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(map[interface{}]interface{}{"key": "val"})))
		})
		...
		It("finds nested matching item", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(map[interface{}]interface{}{"nested-key": "val"})))
		})
		...
		It("finds nested missing matching item if it does not exist", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(map[interface{}]interface{}{"name": "val"})))
		})

		It("returns an error if it's not an array is being accessed", func() {
			_, err := FindOp{Path: MustNewPointerFromString("/key=val")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/key=val' but found '*yamltree.ClassicMappingNode'"))

			_, err = FindOp{Path: MustNewPointerFromString("/key=val/items/key=val")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/key=val' but found '*yamltree.ClassicMappingNode'"))
	...
	Describe("map key", func() {
		It("finds map key", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2("abc")))
		})
		...
		It("finds nested map key", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2("efg")))
		})
		...
		It("returns an error if it's not a map when key is being accessed", func() {
			_, err := FindOp{Path: MustNewPointerFromString("/abc")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find a map at path '/abc' but found '*yamltree.ClassicSequenceNode'"))

			_, err = FindOp{Path: MustNewPointerFromString("/abc/efg")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find a map at path '/abc' but found '*yamltree.ClassicSequenceNode'"))
