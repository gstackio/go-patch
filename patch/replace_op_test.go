...
var _ = Describe("ReplaceOp.Apply", func() {
	It("returns error if replacement value cloning fails", func() {
		_, err := ReplaceOp{Path: MustNewPointerFromString(""), Value: func() {}}.Apply(CreateYamlNodeV2("a"))
	...
	It("uses cloned value for replacement", func() {
		...
		res, err := ReplaceOp{Path: MustNewPointerFromString(""), Value: repVal}.Apply(CreateYamlNodeV2("a"))
		Expect(err).ToNot(HaveOccurred())
		Expect(res).To(Equal(CreateYamlNodeV2(repVal)))

		res.(YamlMapping).Replace("c", CreateYamlNodeV2("d"))
		Expect(res).ToNot(Equal(CreateYamlNodeV2(repVal)))
	})

	It("replaces document if path is for the entire document", func() {
		res, err := ReplaceOp{Path: MustNewPointerFromString(""), Value: "b"}.Apply(CreateYamlNodeV2("a"))
		Expect(err).ToNot(HaveOccurred())
		Expect(res).To(Equal(CreateYamlNodeV2("b")))
	})

	Describe("array item", func() {
		It("replaces array item", func() {
			res, err := ReplaceOp{Path: MustNewPointerFromString("/0"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{10, 2, 3})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/1"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, 10, 3})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/2"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, 2, 10})))
		})

		It("replaces relative array item", func() {
			res, err := ReplaceOp{Path: MustNewPointerFromString("/3:prev"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, 2, 10})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/0:before"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{10, 1, 2, 3})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/1:before"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, 10, 2, 3})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/3:prev:after"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, 2, 3, 10})))
		})

		It("replaces nested array item", func() {
			doc := CreateYamlNodeV2([]interface{}{[]interface{}{10, 11, 12}, 2, 3})

			res, err := ReplaceOp{Path: MustNewPointerFromString("/0/1"), Value: 100}.Apply(doc)
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{[]interface{}{10, 100, 12}, 2, 3})))
		})

		It("replaces relative nested array item", func() {
			doc := CreateYamlNodeV2([]interface{}{1, []interface{}{10, 11, 12}, 3})

			res, err := ReplaceOp{Path: MustNewPointerFromString("/0:next/1"), Value: 100}.Apply(doc)
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, []interface{}{10, 100, 12}, 3})))
		})
		...
		It("returns an error if it's not an array when index is being accessed", func() {
			_, err := ReplaceOp{Path: MustNewPointerFromString("/0")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/0' but found '*yamltree.ClassicMappingNode'"))

			_, err = ReplaceOp{Path: MustNewPointerFromString("/0/1")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/0' but found '*yamltree.ClassicMappingNode'"))
		...
		It("returns an error if the index is out of bounds", func() {
			_, err := ReplaceOp{Path: MustNewPointerFromString("/1")}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find array index '1' but found array of length '0' for path '/1'"))

			_, err = ReplaceOp{Path: MustNewPointerFromString("/1/1")}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find array index '1' but found array of length '0' for path '/1'"))
	...
	Describe("array with after last item", func() {
		It("appends new item", func() {
			res, err := ReplaceOp{Path: MustNewPointerFromString("/-"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{10})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/-"), Value: 10}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{1, 2, 3, 10})))
		})
		...
		It("appends nested array item", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{[]interface{}{10, 11, 12, 100}, 2, 3})))
		})
		...
		It("returns an error if after last index token is not last", func() {
			...
			_, err := ReplaceOp{Path: ptr}.Apply(CreateYamlNodeV2([]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
		...
		It("returns an error if it's not an array being accessed", func() {
			_, err := ReplaceOp{Path: MustNewPointerFromString("/-")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/-' but found '*yamltree.ClassicMappingNode'"))

			doc := CreateYamlNodeV2(map[interface{}]interface{}{"key": map[interface{}]interface{}{}})

			_, err = ReplaceOp{Path: MustNewPointerFromString("/key/-")}.Apply(doc)
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/key/-' but found '*yamltree.ClassicMappingNode'"))
		})
	})
	...
	Describe("array item with matching key and value", func() {
		...
		It("replaces array item even if not all items are maps", func() {
			...
			res, err := ReplaceOp{Path: MustNewPointerFromString("/key=val"), Value: 100}.Apply(doc)
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2([]interface{}{3, 100})))
		})
		...
		It("appends missing matching item if it does not exist", func() {
			doc := CreateYamlNodeV2([]interface{}{map[interface{}]interface{}{"xyz": "xyz"}})
		...
		It("returns an error if it's not an array is being accessed", func() {
			_, err := ReplaceOp{Path: MustNewPointerFromString("/key=val")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/key=val' but found '*yamltree.ClassicMappingNode'"))

			_, err = ReplaceOp{Path: MustNewPointerFromString("/key=val/items/key=val")}.Apply(CreateYamlNodeV2(map[interface{}]interface{}{}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find an array at path '/key=val' but found '*yamltree.ClassicMappingNode'"))
	...
	Describe("map key", func() {
		It("replaces map key", func() {
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(map[interface{}]interface{}{"abc": 1, "xyz": "xyz"})))

			res, err = ReplaceOp{Path: MustNewPointerFromString("/abc")}.Apply(doc)
			Expect(err).ToNot(HaveOccurred())
			Expect(res).To(Equal(CreateYamlNodeV2(map[interface{}]interface{}{"abc": nil, "xyz": "xyz"})))
		})
		...
		It("returns an error if it's not a map when key is being accessed", func() {
			_, err := ReplaceOp{Path: MustNewPointerFromString("/abc")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find a map at path '/abc' but found '*yamltree.ClassicSequenceNode'"))

			_, err = ReplaceOp{Path: MustNewPointerFromString("/abc/efg")}.Apply(CreateYamlNodeV2([]interface{}{1, 2, 3}))
			Expect(err).To(HaveOccurred())
			// fmt.Fprintf(os.Stderr, "\nError found: [%s]\n", err.Error())
			Expect(err.Error()).To(Equal(
				"Expected to find a map at path '/abc' but found '*yamltree.ClassicSequenceNode'"))
